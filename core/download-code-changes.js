...
/*startCrawling(projectApiUrl).then(() => {
...
async function startCrawling(projectApiUrl) {
    ...
    return Promise.all([openChangePromise, mergedChangePromise, abandonedChangedPromise]).then(() => {
...
function saveFiles(changeUrlString, json) {
    ...
    let dirname = racine + (changeUrl.hostname.split("."))[1];
