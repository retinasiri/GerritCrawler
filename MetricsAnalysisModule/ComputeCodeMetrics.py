...
def downloadCodeFetch(data):
    ...
    repoName = REPOSITORIES_PATH + urlparse.urlsplit(fetch_url).path
    commandToExec = "cd " + repoName + " && " + "git fetch " + fetch_url + " " + fetch_ref

    print(commandToExec)

    os.system(commandToExec)
    getCodeMetrics(repoName, commitHash)
